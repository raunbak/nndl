...
def test_Sigmoid_func_zero_value_output():
    ...
    assert output == np.array([[0.5]]), "Sigmoid must equal 0.5 at z = 0.0"
...
def test_Sigmoid_func_prime_output():
    ...
    assert np.array_equal(
        Sigmoid().func_prime(z=z), np.array([[0.25, 0.25, 0.25]]).T
    ), "Sigmoid prim_func with input vector [0,0,0].T must equal [0.25, 0.25, 0.25]"


def test_ReLU_class_type():
    assert isinstance(ReLU(), NeuronBase), "ReLU must be an instance of NeuronBase"


def test_Tanh_class_type():
    assert isinstance(Tanh(), NeuronBase), "Tanh must be an instance of NeuronBase"
